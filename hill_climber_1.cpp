        ...
        for (auto& n : nodes) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                if (nodes_map.count({c_test, n.N})) {
                    ...
                    // FIXED VECTOR MATH: Halve the vectors before subtracting to find the corner
                    ...
                    if (!nodes_map.count({corner_test, vec})) {
                        cout << "\nCRITICAL ERROR: Mathematical hole in topology at Box " 
                             << W << "x" << H << "x" << D << endl;
                        exit(1);
                    }
...
// Add this function right above your main() function
void print_success(vector<NetState>& nets) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // Find boundaries for the ASCII grid
        ...
        for (int y = min_y; y <= max_y; ++y) {
            for (int x = min_x; x <= max_x; ++x) {
                int node_id = -1;
                // Find which node occupies this 2D coordinate
...
// Replace your entire main() function with this updated version
int main() {
    ...
    vector<NetState> nets = {{&b1}, {&b2}, {&b3}, {&b4}};
    
    cout << "Generating deterministic valid starting nets (70 squares)..." << endl;
    for (int i = 0; i < 4; ++i) nets[i].generate_seed();
    ...
    int max_possible_score = 6 * 70; // 6 pairs, perfect overlap is 70 per pair (420)
    ...
    long long iterations_since_improvement = 0; // Tracks our plateau
    ...
    while (current_score < max_possible_score) {
        ...
        while (!nets[target].mutate()) {
            attempt++;
            if (attempt > 200000) attempt = 0; 
        }

        int new_score = evaluate_fitness(nets, transB, touched);

        // Accept equal scores to wander across plateaus
        if (new_score >= current_score) {
            if (new_score > current_score) {
                iterations_since_improvement = 0; // Reset plateau timer!
            } else {
        ...
        // --- THE PLATEAU BREAKER ---
        // If we've been wandering aimlessly on a flat plateau for too long, force a reset
        if (iterations_since_improvement > 300000) {
            cout << "\n*** PLATEAU DETECTED *** Nuking Box " << target << " to break the deadlock!\n" << endl;
            nets[target].generate_seed(); // Completely randomizes this box
